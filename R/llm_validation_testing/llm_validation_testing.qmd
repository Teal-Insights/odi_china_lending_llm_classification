...
```
# Time
Processing time for 300 validation 
(data from processing_log.txt in data/validation_results for each LLM)
```{r}
...
# Theme settings for consistent, professional visualization
...
```{r}
library(tidyverse)
library(openxlsx)  # For Excel output with formatting
